...
usage() {
    ...
    echo ' -s Execute the COMMAND using sudo on the remote serer.' >&2
    echo ' -v Verbose mode. Displays the server name before executing COMMAND.' >&2
    exit 1
}

# Make sure the script begins execution without superuser privileges.
if [[ "${UID}" -eq 0 ]]
then 
...
# Remove the options while leaving the remaining arugments.
shift "(( OPTIND -1 ))"
